/*gcc `pkg-config --cflags --libs gtk+-2.0` GTK_start.c -o GTK_start -lPJ_RPI*/
...
int main (int argc, char *argv[])
{
    ...
    GtkWidget *lbl = gtk_label_new ("My label");//label 
    ...
    gtk_table_attach_defaults (GTK_TABLE (tbl), lbl, 1, 3, 0, 1); // 1ste nummer < 2de nummer, 3de nummer < 4de nummer (eerste 2 zijn de breedte, laatste 2 zijn de hoogte)
    gtk_table_attach_defaults (GTK_TABLE (tbl), btn2, 0, 4, 1, 3); 
    gtk_table_attach_defaults (GTK_TABLE (tbl), btn, 3, 4, 3, 4);
